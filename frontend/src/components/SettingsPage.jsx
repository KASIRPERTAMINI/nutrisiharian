...
const SettingsPage = ({ onBack, currentTargets, onSave, onReset }) => {
  ...
  const [showConfirmModal, setShowConfirmModal] = useState(0); // 0 = sembunyi, 1 = konfirmasi pertama, 2 = konfirmasi kedua
  ...
  const handleResetClick = () => {
    setShowConfirmModal(1); // Tampilkan konfirmasi pertama
  };

  const handleConfirmReset = () => {
    if (showConfirmModal === 1) {
      setShowConfirmModal(2); // Tampilkan konfirmasi kedua
    } else if (showConfirmModal === 2) {
      onReset(); // Panggil fungsi reset dari App.jsx
